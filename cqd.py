...
class CQD(nn.Module):
    ...
    def forward(self,
        ...
        for query_structure, queries in batch_queries_dict.items():
            ...
            if 'co' in self.method:
                ...
                if graph_type == "1p":
                    ...
                    #DM hack - need to check all scores
                    scores, scores_1, scores_2, scores_1_original, scores_2_original, = d2.query_2in(temp1=self.temp1,temp2=self.temp2,entity_embeddings=self.embeddings[0],
                                          predicate_embeddings=self.embeddings[1],
                                          queries=queries,
                                          scoring_function=scoring_function,
                                          t_norm=t_norm, negation=negation)
                    ...
                    scores, scores_1, scores_2, scores_1_original, scores_2_original, = d2.query_3in(temp1=self.temp1,temp2=self.temp2,entity_embeddings=self.embeddings[0],
                                        predicate_embeddings=self.embeddings[1],
                                        queries=queries,
                                        scoring_function=scoring_function,
                                        t_norm=t_norm, negation=negation)                  
                    ...
                    scores, scores_1, scores_2, scores_1_original, scores_2_original,  = d2.query_pin(temp1=self.temp1,temp2=self.temp2,entity_embeddings=self.embeddings[0],
                                          predicate_embeddings=self.embeddings[1],
                                          queries=queries,
                                          scoring_function=scoring_function,
                    ...
                    scores, scores_1, scores_2, scores_1_original, scores_2_original, = d2.query_inp(temp1=self.temp1,temp2=self.temp2,entity_embeddings=self.embeddings[0],
                                          predicate_embeddings=self.embeddings[1],
                                          queries=queries,
                                          scoring_function=scoring_function,
                                          t_norm=t_norm, negation=negation)                    
        ...
        return None, scores, scores_1, scores_2, scores_1_original, scores_2_original, None, all_idxs
