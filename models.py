...
class KGReasoning(nn.Module):
    ...
    def test_step(model, easy_answers, hard_answers, args, test_dataloader, query_name_dict, save_result=False, save_str="", save_empty=False):
        ...
        torch.set_printoptions(precision=10,sci_mode=True, threshold=0,edgeitems=321)
        ...
        with torch.set_grad_enabled(isinstance(model, CQD) and model.method == 'co'):
            for negative_sample, queries, queries_unflatten, query_structures in tqdm(test_dataloader, disable=not args.print_on_screen):
                ...
                #DM hack - need to see both scores
                # _, negative_logit, _, idxs = model(None, negative_sample, None, batch_queries_dict, batch_idxs_dict)              
                _, negative_logit, scores_1, scores_2, scores_1_original, scores_2_original, _, idxs = model(None, negative_sample, None, batch_queries_dict, batch_idxs_dict)      
                ...
                # gc+=1 
                # if gc==1:
                #     #index is argsort and the source is model.batch_entity_range
                #     print('')
                #     print('ranking2',ranking)
                #     print('argmax',torch.argmax(ranking),ranking.size())
                #     print('max is',torch.max(ranking))
                    
                #     print('model.nentity',model.nentity)
                #     #print('model.batch_entity_range',model.batch_entity_range) #aranged tensor 0 to 14504
                #     print('model.batch_entity_range.size',model.batch_entity_range.size())  #this is 1000x14505
                    
                #     print('arange',torch.arange(model.nentity).to(torch.float).repeat(argsort.shape[0],1))
                ...
                for idx, (i, query, query_structure) in enumerate(zip(argsort[:, 0], queries_unflatten, query_structures)):
                    ...
                    # cur_ranking_temp1 = cur_ranking.clone()       
                    
       
                    cur_ranking, indices = torch.sort(cur_ranking)
                    masks = indices >= num_easy
                                  
                    # cur_ranking_temp2 = cur_ranking.clone()
                    ...
                    if False:
                    # if len(hard_answer)==1 and h1==0:
                        
                        mistakes_total+=1
                                               
                        #ranking is a 14505 tensor corresponding to the ranking of each entity
                        #for example the value of the first element of array ranking is the ranking of the first entity
                        #the model answer is the argument that is equal to the position of the first hard answer. i.e if there are 8 easy answers and 1 hard, we are looking for the argument of the 9th largest ranking
                        #the correct answer is the 

                        # correct_answer = next(iter(hard_answers[query]))
                        correct_answer = list(hard_answers[query])[0]
                        
                        correct_ranking = ranking[0,correct_answer].int()    
            
                        model_ranking = cur_ranking[0].int()        
                        # model_ranking = cur_ranking.int()        
                    
                        model_answer = (ranking[0,:] == model_ranking).nonzero(as_tuple=True)[0].cpu().numpy()[0]
                        #model_answer=ranking[0,model_answer]  #similar as above
               

                        
                        # model_answer = ranking[0,model_ranking].long()
                        mistake = correct_ranking - model_ranking
                        ...
                        query1_correct = ( str(query[0][0]),str(query[0][1][0]),str(correct_answer) )
                        query2_correct = ( str(query[1][0]),str(query[1][1][0]),str(correct_answer) )
                        query1_model   = ( str(query[0][0]),str(query[0][1][0]),str(model_answer)   )
                        query2_model   = ( str(query[1][0]),str(query[1][1][0]),str(model_answer)   )

                        print('hits@1  score',h1)
                        print('hits@3  score',h3)
                        print('hits@10 score',h10)
                        
                        print('list hard_answers',(list(hard_answers[query])))
                        print('list easy_answers',(list(easy_answers[query])))
                        print('len list hard_answers[query]',len(list(hard_answers[query])))
                        print('len list easy_answers[query]',len(list(easy_answers[query])))
                        
                        print('')
                        print('correct hard answer           ',correct_answer)  
                        print('ranking of correct hard answer  ', correct_ranking)                                         
                        print('score_1           of correct answer',scores_1[0,correct_answer])
                        print('score_2           of correct answer',scores_2[0,correct_answer])
                        print('score_1 * score_2 of correct answer',scores_1[0,correct_answer]*scores_2[0,correct_answer])                     
                        
                        print('')
                        print('model hard answer             ', model_answer)
                        print('ranking of model   hard answer', model_ranking)                                         
                        print('score_1           of model   answer',scores_1[0,model_answer])
                        print('score_2           of model   answer',scores_2[0,model_answer])
                        print('score_1 * score_2 of model   answer',scores_1[0,model_answer] * scores_2[0,model_answer])                        

                        print('')
                        print('ranking is off by             ', mistake)
                        ...
                        print('query1_correct',query1_correct)
                        if query1_correct in train_triples:
                            print('query1_correct found in train file')
                        else:
                            print('query1_correct not found in train file')
                        if query1_correct in test_triples:
                            print('query1_correct found in test file')
                        else:
                            print('query1_correct not found in test file')
                        if query1_correct in valid_triples:
                            print('query1_correct found in valid file')
                        else:
                            print('query1_correct not found in valid file')
 
                        print('-'*100)
                            
                        print('query2_correct',query2_correct)
                        if query2_correct not in train_triples:
                            print('query2_correct not found in train file')
                        else:
                            print('query2_correct found in train file')
                        if query2_correct not in test_triples:
                            print('query2_correct not found in test file')
                        else:
                            print('query2_correct found in test file')
                        if query2_correct not in valid_triples:
                            print('query2_correct not found in valid file')
                        else:
                            print('query2_correct found in valid file')
                        
                        print('-'*100)
                        
                        
                        #search
                        #for left querries confirm they appear at least once, problem = 0
                        #for right querries confirm they do not appear everywhere, problem2 > 0
                        
                        query1=0
                        query2=0
                        
                        print('query1_model',query1_model)
                        if query1_model in train_triples:
                            print('query1_model found in train file')
                            query1+=1    
                        else:
                            print('query1_model not found in train file')
                        if query1_model in test_triples:
                            print('query1_model found in test file')
                            query1+=1
                        else:
                            print('query1_model not found in test file')
                        if query1_model in valid_triples:
                            print('query1_model found in valid file')
                            query1+=1
                        else:
                            print('query1_model not found in valid file')

                        print('-'*100)
                                             
                        print('query2_model',query2_model)
                        if query2_model not in train_triples:
                            print('query2_model not found in train file')
                        else:
                            print('query2_model found in train file')
                            query2+=1
                        if query2_model not in test_triples:
                            print('query2_model not found in test file')
                        else:
                            print('query2_model found in test file')
                            query2+=1
                        if query2_model not in valid_triples:
                            print('query2_model not found in valid file')
                        else:
                            print('query2_model found in valid file')
                            query2+=1
                        ...
                        if query1==0 and query2==0:
                            print('query1 is the only issue, values are', query1, query2)
                            q1.append([gc,query1,query2])

                        if query1>0 and query2>0:
                            print('query2 is the only issue, values are', query1, query2)
                            q2.append([gc,query1,query2])

                        if query1==0 and query2>0:
                            print('both queries are the issue, values are', query1, query2)
                            q3.append([gc,query1,query2])
                            
                        if query1>0 and query2==0:
                            print('cannot determine which query is the issue, values are', query1, query2)
                            q4.append([gc,query1,query2])

                        if query1==0:
                            q1_cumulative.append([gc,query1])
                        
                        if query2>0:
                            q2_cumulative.append([gc,query2])
                        ...
                        # print('sum scores_1',torch.sum(scores_1))
                        # print('min scores_1',torch.min(scores_1))
                        # print('max scores_1',torch.max(scores_1))

                        # print('scores_2',scores_2)                        
                        # print('sum scores_2',torch.sum(scores_2))
                        # print('min scores_2',torch.min(scores_2))                        
                        # print('max scores_2',torch.max(scores_2))
                        # print('-'*100)              
                                                
                        # print('scores_1_original',scores_1_original)
                        # print('sum scores_1_original',torch.sum(scores_1_original))
                        # print('min scores_1_original',torch.min(scores_1_original))
                        # print('max scores_1_original',torch.max(scores_1_original))
                        
                        # print('scores_2_original',scores_2_original)
                        # print('sum scores_2_original',torch.sum(scores_2_original))
                        # print('min scores_2_original',torch.min(scores_2_original))                        
                        # print('max scores_2_original',torch.max(scores_2_original))
                        
                        # print('-'*100)
                        # print('-'*100)
                        
                        
                        # results_list.append( [ gc,query,query1,query2, scores_1[0,correct_answer],scores_2[0,correct_answer],scores_1[0,model_answer],scores_2[0,model_answer] ] )
                        # results_list.append( [ gc,query1,query2 ] )
        ...
        #added by DM print logs
        # print('-'*100)
        # print('total mistakes',mistakes_total)
        
        # print('Presentation A - each set is unique')
        # print('query1 is the only issue (query1==0 and query2==0)',len(q1))
        # print('query2 is the only issue (query1>0 and query2>0)',len(q2))
        # print('both queries are the issue (query1==0 and query2>0)',len(q3))
        # print('cannot determine the issue (query1>0 and query2==0)',len(q4))

        # print('Presentation B - cumulative counting the same example may appear in both lists')
        # print('q1_cumulative (query1==0)',len(q1_cumulative))
        # print('q2_cumulative (query2>0)',len(q2_cumulative))
        ...
        for query_structure in logs:     
            ...
            print('@',metrics[query_structure]['MRR'])
            print('@',metrics[query_structure]['HITS1'])
            print('@',metrics[query_structure]['HITS3'])
            print('@',metrics[query_structure]['HITS10'])
